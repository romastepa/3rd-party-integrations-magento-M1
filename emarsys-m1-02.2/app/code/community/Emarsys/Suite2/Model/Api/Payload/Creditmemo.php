    ...
    protected function _generateItems()
    {
        ...
        foreach ($this->_creditmemo->getAllItems() as $item) {
            ...
            if ($rowTotal = $useBaseCurrency ? $item->getBaseRowTotalInclTax() : $item->getRowTotalInclTax()) {
                ...
                $data[] = array(
                    ...
                    'unit_price'    => -1 * $this->_formatPrice($useBaseCurrency ? $item->getBasePriceInclTax() : $item->getPriceInclTax()),
                    'c_sales_amount'=> -1 * $this->_formatPrice(-($rowTotal - $discountAmount)),
                );
        ...
        if ($useBaseCurrency ? (float)$this->_creditmemo->getBaseAdjustment() : (float)$this->_creditmemo->getAdjustment()) {
            $data[] = array(
                    ...
                    'item'          => 0,
                    'quantity'      => -1,
                    'unit_price'    => -1 * $this->_formatPrice($useBaseCurrency ? $this->_creditmemo->getBaseAdjustment() : $this->_creditmemo->getAdjustment()),
                    'c_sales_amount'=> -1 * $this->_formatPrice($useBaseCurrency ? -$this->_creditmemo->getBaseAdjustment() : -$this->_creditmemo->getAdjustment()),
                );
        }
